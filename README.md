...
动态业务 API 的成功响应统一为 `{"code": 0, "message": "OK", "data": ...}`；生成处理器的返回值位于 `data`。管理 API 和健康检查维持各自原有契约。
